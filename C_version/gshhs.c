/*	$Id: gshhs.c,v 1.45 2011/07/01 18:01:38 guru Exp $
 *
...
int main (int argc, char **argv)
{
	...
	FILE *fp = NULL;
	int k, line, max_east = 270000000, info, single, error, ID, flip, m, octave = 0;
	...
	if (argc == 1 || error) {
		...
		fprintf (stderr, "usage:  gshhs gshhs_[f|h|i|l|c].b [-A<area>] [-G] [-I<id>] [-L] [-M] > ascii.txt\n\n");
	...
	while (n_read == 1) {
		...
		OK = ((!single || h.id == ID) && area >= minarea);
		first = 0;
