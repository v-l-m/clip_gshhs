...
void GshhsToGpc(gshhs_polygons *GshhsPolygon, gpc_polygon *GpcPolygon, int GshhsIdPoly)
{
    ...
    for (v= 0; v < num_vertices; v++)
    {
        ...
        GpcPolygon->contour[0].vertex[v].y = GshhsPolygon->contour[GshhsIdPoly].vertex[v].y;
    }
}

void CreateClip(gpc_polygon *GpcPolygon, double xmin, double ymin, double xmax, double ymax)
{

    MALLOC(GpcPolygon->hole, 1 * sizeof(int), "hole flag array creation", int);
    MALLOC(GpcPolygon->contour, 1 * sizeof(gpc_vertex_list), "contour creation", gpc_vertex_list);
    MALLOC(GpcPolygon->contour[0].vertex, 4 * sizeof(gpc_vertex), "vertex creation", gpc_vertex);
    ...
    GpcPolygon->contour[0].vertex[2].x = xmax;
    GpcPolygon->contour[0].vertex[2].y = ymax;

    GpcPolygon->contour[0].vertex[3].x = xmin;
    GpcPolygon->contour[0].vertex[3].y = ymax;
...
void PolyDataCreate(PolyData *Data, int step)
{
    ...
    for(i=0; i<4; i++)
    {
        Data->poly[i] = malloc((360/step) * sizeof(gpc_polygon**));
        if (Data->poly[i]==NULL)
        {
            fprintf(stderr, "Memory allocation failed\n");
            exit (EXIT_FAILURE);
        ...
        {
            Data->poly[i][j] = malloc((180/step) * sizeof(gpc_polygon*));
            if (Data->poly[i][j]==NULL)
            {
                fprintf(stderr, "Memory allocation failed\n");
                exit (EXIT_FAILURE);
            }
            for(k=0; k<(180/step); k++)
            {
                Data->poly[i][j][k] = malloc(sizeof(gpc_polygon));
                if (Data->poly[i][j][k]==NULL)
                {
                    fprintf(stderr, "Memory allocation failed\n");
                    exit (EXIT_FAILURE);
                }
...
void PolyDataDelete(PolyData *Data)
{
    ...
    for(i=0; i<4; i++)
    {
        for(j=0; j<(360/Data->step); j++)
        {
            for(k=0; k<(180/Data->step); k++)
            {
                gpc_free_polygon(Data->poly[i][j][k]);
            }
           free(Data->poly[i][j]);
           Data->poly[i][j]=NULL;
        }
...
void ReClipping(PolyData *DataIn, PolyData *DataOut)
{
    gpc_polygon clip;
    gpc_polygon result;
...
void *ReClippingM(void *p_data)
{
    ...
    {
        //printf("Level: %d\n", level);
        for (x=0; x<(360/data->DataOut->step); x++)
        {
            //printf("th: %d /t x: %d\n", data->th_num, x);
            for (y=0; y<(180/data->DataOut->step); y++)
            {
                poly = x*(180/data->DataOut->step) + y;
...
void read_gshhs(FILE *gshhs_file, gshhs_polygons *polygons)
{
    ...
    while (n_read == 1)
    {
        ...
        polygons->contour[h.id].greenwich   = (h.flag >> 16) & 3;   /* greenwich is 0-3 */
        polygons->contour[h.id].source      = (h.flag >> 24) & 1;   /* source is 0 (WDBII) or 1 (WVS) */
        ...
        polygons->contour[h.id].area        = h.area / scale;
        polygons->contour[h.id].area_full   = h.area / scale;
...
{
    int k;
    /* vidage de la memoire des donn\E9es GSHHS */
    for (k = 0; k <= nb_poly; k++)
    {
...
void ReadPolygonFile (FILE *polyfile,
                        int x, int y,
                        int pas_x, int pas_y,
                        gpc_polygon *p1, gpc_polygon *p2, gpc_polygon *p3, gpc_polygon *p4)
{
...
void DegToHMS(char *hms, double n, char *type)
{
    ...
    if (n>=0)
    {
        ...
        if (strcmp(type, "long") == 0)  sprintf(hms, "%d°0%.2d'%.2d\"E", deg, min, sec);
        if (strcmp(type, "lat") == 0)   sprintf(hms, "%d°0%.2d'%.2d\"N", deg, min, sec);
    ...
    else
    {
        ...
        min=floor((n-deg)*60);
        sec=((n-floor(n))*60-min)*60;
        if (strcmp(type, "long") == 0)  sprintf(hms, "%d°0%.2d'%.2d\"W", deg, min, sec);
        if (strcmp(type, "lat") == 0)   sprintf(hms, "%d°0%.2d'%.2d\"S", deg, min, sec);
    }
...
*/
    ...
    // Ouverture du fichier de donnees altimetrique et lecture du Header
    /*
    ...
    for (lat=0; lat<TileDim; lat++)
    {
        ...
        for (lon=0; lon < TileDim; lon++)
        {
            if (TestImg[lon + TileDim * lat] == 0)
            {
                ...
                // Longitude et latitude du du point de donnee ETOPO le plus proche
